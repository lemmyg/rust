...
fn initialize_layers() -> Vec<Button>{
...
fn main() {
    ...
    let mut layers: Vec<Button> = initialize_layers();
    for button in layers {
        println!("looping: key {}: with value {:?}", button.label, button.action );
    }
